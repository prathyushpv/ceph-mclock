...
//////////////////// Modified mClock Scheduler//////////////////////////////



ModifiedmClockScheduler::ModifiedmClockScheduler(CephContext *cct) :
  scheduler(
    std::bind(&ModifiedmClockScheduler::ClientRegistry::get_info,
	      &client_registry,
	      _1),
    dmc::AtLimit::Wait,
    cct->_conf.get_val<double>("osd_mclock_scheduler_anticipation_timeout"))
...
void ModifiedmClockScheduler::ClientRegistry::update_from_config(const ConfigProxy &conf)
{
  default_external_client_info.update(
    conf.get_val<uint64_t>("osd_mclock_scheduler_client_res"),
    conf.get_val<uint64_t>("osd_mclock_scheduler_client_wgt"),
...
const dmc::ClientInfo *ModifiedmClockScheduler::ClientRegistry::get_external_client(
  const client_profile_id_t &client) const
{
  auto ret = external_client_infos.find(client);
  if (ret == external_client_infos.end())
...
const dmc::ClientInfo *ModifiedmClockScheduler::ClientRegistry::get_info(
  const scheduler_id_t &id) const {
  switch (id.class_id) {
  case op_scheduler_class::immediate:
    ceph_assert(0 == "Cannot schedule immediate");
...
void ModifiedmClockScheduler::dump(ceph::Formatter &f) const
{
}

void ModifiedmClockScheduler::enqueue(OpSchedulerItem&& item)
{
  auto id = get_scheduler_id(item);
  // TODO: express cost, mclock params in terms of per-node capacity?
  auto cost = 1; //std::max(item.get_cost(), 1);
...
void ModifiedmClockScheduler::enqueue_front(OpSchedulerItem&& item)
{
  immediate.push_back(std::move(item));
  // TODO: item may not be immediate, update mclock machinery to permit
  // putting the item back in the queue
...
OpSchedulerItem ModifiedmClockScheduler::dequeue()
{
  if (!immediate.empty()) {
    auto ret = std::move(immediate.back());
    immediate.pop_back();
...
const char** ModifiedmClockScheduler::get_tracked_conf_keys() const
{
  static const char* KEYS[] = {
    "osd_mclock_scheduler_client_res",
    "osd_mclock_scheduler_client_wgt",
...
void ModifiedmClockScheduler::handle_conf_change(
  const ConfigProxy& conf,
  const std::set<std::string> &changed)
{
  client_registry.update_from_config(conf);
